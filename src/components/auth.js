...
const Auth = () => {
  ...
  const [showSignUp, setShowSignUp] = useState(false); // State to toggle SignUp component visibility
  ...
  const handleSignUpSuccess = async (user) => {
    setShowSignUp(false); // Hide the SignUp component after successful sign up
    setUser(user);
    setIsSignUp(false);
  };
    ...
    <div>
      {user ? (
        ...
        <div>
          ...
          {!showSignUp && ( // Render SignUp link if showSignUp is false
            <p>
              Don't have an account?{' '}
              <button onClick={handleSignUp}>Sign Up!</button>
            </p>
          )}
          {showSignUp && ( // Render SignUpForm if showSignUp is true
